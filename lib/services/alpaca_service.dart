...
class AlpacaService {
  ...
  Future<List<ChartDataPoint>> getChartDataPoints(String symbol) async {
    final url = Uri.parse(
        '${Constants.alpacaBaseUrl}/v2/stocks/$symbol/bars?timeframe=30Min&start=2024-06-15T00%3A00%3A00Z&end=2024-12-31T23%3A59%3A00Z&limit=10000&adjustment=raw&feed=iex&sort=asc');
    final response = await http.get(url, headers: {
      'APCA-API-KEY-ID': _apiKey,
      'APCA-API-SECRET-KEY': _apiSecret,
    });
